...
def loop_drive( ID ):
    ...
    while(1):
        ...
        #print(myThetaDot)
        #Theta dot is the rotational speed that the SCUTTLE needs to go
        # Speed in m/s of each wheel that you need to do
        #myXDot = 0 #enable this line to freeze driving, and turn only.
        #myXDot is the speed in rad/s
        #myXDot = (-0.01)*(colorTarget[2]-120)
        myXDot = .25
        A = np.array([ myXDot, myThetaDot ])
        #pdTargets is generated from (xdot, thetadot) after going through inverse kinematics
        pdTargets = inv.convert(A) # convert [xd, td] to [pdl, pdr] TODO: how is pdTargets calculating pdTargets?
        #print(pdTargets[0])
        #print(pdTargets[1])

        #THIS BLOCK IS FOR ASSESSING ACTION TO TAKE FOR COLOR TRACKING
        #print(myXDot)

        #if myThetaDot < -0.5:
        #    print("The target is: ",myThetaDot," to the right.")
        #elif myThetaDot > 0.5:
        #    print("The target is: ",myThetaDot," to the left.")
        #else:
        #    print("The target is centered.")


        print("X: ",colorTarget[0],"\t","Y: ",colorTarget[1],"\t","Radius: ",colorTarget[2],"\t")
        file = open('/home/debian/ReadFile.txt', 'w+')
        file.write(str(colorTarget[2]))
        file.close()
        ...
        # time.sleep(0.05)
        if 15 <= colorTarget[2] <= 60:
            sc.driveClosedLoop(pdTargets, pdCurrents, de_dt)  # call the control system
            #sc.driveOpenLoop(pdTargets)
        elif colorTarget[2] > 60:
            print("The SCUTTLE has docked.")
            sys.exit()
        elif colorTarget[2] < 15:
            #sys.exit()
            print("Too far.")
